...
function getArgumentsCount(funcs) {
  return funcs.length
    ? funcs.map((x) => {
        const temp = getFunctionBody(x);
        const lenght = temp.length;
        let i;
        let j;
        for (i = 0; i < lenght && temp[i] !== '('; i += 1);
        for (j = i + 1; j < lenght && temp[j] !== ')'; j += 1);
        if (i === lenght || j === lenght || i === j - 1) {
          return 0;
        }
        return temp.substring(i + 1, j).split(',').length;
      })
    : [];
}
...
function getPolynom(...args) {
  return (x) => {
    return args.length
      ? args
          .map((i, pos) => i * x ** (args.length - pos - 1))
          .reduce((y, j) => y + j, 0)
      : null;
  };
}
...
function logger(func, logFunc) {
  return (...value) => {
    let name = getFunctionBody(func).split(' ')[1];
    name = name.substring(0, name.indexOf('('));
    logFunc(`${name}(${JSON.stringify(value).slice(1, -1)}) starts`);
    const result = func(...value);
    logFunc(`${name}(${JSON.stringify(value).slice(1, -1)}) ends`);
    return result;
  };
}
